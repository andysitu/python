...
def findPayment(loan, r, m):
    """ Assumes loan and r are floats, m is an int
        Returns the monthly payment for a mortage of size
        loan at a month rate of r for m months"""
...
class Mortage(object):
    ...
    def plotNet(self, style):
        """ Plot an approximation to the total cost of the mortage
...
class Fixed(Mortage):
    def __init__(self, loan, r, months):
        ...
        self.legend = legend = 'Fixed, ' + str(r*100) + '%'
...
def plotMortages(morts, amt):
    ...
    pylab.title('Month Payments of Different $' + str(amt)
                + ' Mortages')
    pylab.xlabel('Months')
    pylab.ylabel('Monthly Labels')
    pylab.figure(cost)
    pylab.title('Cash Outlay of Different $' + str(amt) + ' Mortages')
    pylab.xlabel('Months')
    pylab.ylabel('Total Payments')
    pylab.figure(balance)
    pylab.title('Balance Remaining of $' + str(amt) + ' Mortages')
    pylab.xlabel('Months')
    pylab.ylabel('Remaining Loan Balance of $')
    pylab.figure(netCost)
    pylab.title('Net Cost of $' + str(amt) + ' Mortages')
    pylab.xlabel('Months')
